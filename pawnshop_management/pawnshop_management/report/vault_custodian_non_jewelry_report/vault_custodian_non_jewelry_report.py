...
def execute(filters=None):
	columns, data = [], []
	columns = get_columns()
	data2 = frappe.get_all('Pawn Ticket Non Jewelry', filters=filters, fields=['date_loan_granted'], order_by='date_loan_granted desc')
	for i in range(len(data2)):
		if not date_has_duplicate(data2[i]['date_loan_granted'], data):
			...
			renewed_count_of_the_day = frappe.db.count('Pawn Ticket Jewelry', {'date_loan_granted': data2[i]['date_loan_granted'], 'workflow_state': 'Renewed'})

			out_count = frappe.db.count('Pawn Ticket Jewelry', {'date_loan_granted': data2[i]['date_loan_granted'], 'workflow_state': 'Redeemed'})

			total_count = frappe.db.count('Pawn Ticket Jewelry', {'date_loan_granted': ['<=', data2[i]['date_loan_granted']], 'workflow_state':['in',['Active', 'Expired']]})

			total_renewed = frappe.db.count('Pawn Ticket Jewelry', {'date_loan_granted': ['<=', data2[i]['date_loan_granted']], 'workflow_state':'Renewed'})
